<template>
    ...
    <!-- <div v-for="item in catalog_Api">
        <v-card class="mx-auto" width="300">
            <v-list>
                <v-list-item :title="item.name"></v-list-item>
                <v-list-group>
                    <template v-slot:activator="{ props }">
                        <v-list-item v-bind="props" title="Danh sách thể loại"></v-list-item>
                    </template>
                    <v-list-item :title="i.name" v-for="i in item.sub"></v-list-item>
                </v-list-group>
            </v-list>
        </v-card>
    </div> -->
    ...
    <v-card class="mx-auto" width="300">
        <v-list>
            ...
            <v-list-group>
                <template>
                    <v-list-item title="Danh sách thể loại"></v-list-item>
                </template>
                <v-list-item title="i.name"></v-list-item>
            </v-list-group>
        </v-list>
    </v-card>
        ...
        <v-expansion-panel v-for="item in catalog_Api" :key="item.id" v-bind:value="item" :title="item.name">
            <v-list lines="one" bg-color="#FFFBE6">
                <v-list-item v-for="i in item.sub" :key="i.id" v-bind:value="i.name">
                    ...
                    <v-btn variant="outlined" size="x-small" icon color="primary" style="margin-right: 10px;">
        ...
        <v-expansion-panel v-for="item in gene_Api" :key="item.id">
            ...
            <v-expansion-panel-text>
                ...
                <v-list lines="one">
                    <v-list-item v-for="i in item.sub" :key="i.id" v-bind:value="i.name">
                        {{ i.name }}
                        <span type="button" @click="" class="badge bg-danger">X</span>
                    </v-list-item>
                </v-list>
            </v-expansion-panel-text>
        </v-expansion-panel>
